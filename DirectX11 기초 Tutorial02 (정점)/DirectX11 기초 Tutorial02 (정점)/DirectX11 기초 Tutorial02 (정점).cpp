// DirectX11 기초 Tutorial02 (정점).cpp : 애플리케이션에 대한 진입점을 정의합니다.
//

#include "framework.h"
#include "DirectX11 기초 Tutorial02 (정점).h"
#include <d3d11.h>
                     ...
                     _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_DIRECTX11TUTORIAL02, szWindowClass, MAX_LOADSTRING);
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_DIRECTX11TUTORIAL02));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_DIRECTX11TUTORIAL02));
    wcex.hCursor        = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground  = (HBRUSH)(COLOR_WINDOW+1);
    wcex.lpszMenuName   = MAKEINTRESOURCEW(IDC_DIRECTX11TUTORIAL02);
...
HRESULT InitDevice()
{
    ...
    //--------- 위에서 선언한 struct SimpleVertex  이 형식으로 버텍스 버퍼를 만들어서 그래픽카드에 넘겨줄 건데 
    //그때 Tutorial02.fx 파일의 버텍스 쉐이더에서 받는 형식과 일치 하는지 확인 하는 부분...
    ...
    //CreateInputLayout 에서 D3D11_INPUT_ELEMENT_DESC 과 vertext shader semantic의 매칭이 이루어짐
    ...
    // Set the input layout    //버텍스 버퍼의 형식은 일치 하니까 이상없어라고 그래픽카드에 정보를 넘겨주는 부분
    g_pImmediateContext->IASetInputLayout(g_pVertexLayout); //일치하니까 g_pImmediateContext<-- 그패픽 디바이스 겍체에 해당됨

    //위에서 선언한 struct SimpleVertex  이 형식으로 버텍스 버퍼를 만들어서 그래픽카드에 넘겨줄 건데 
    //--------- 그때 Tutorial02.fx 파일의 버텍스 쉐이더에서 받는 형식과 일치 하는지 확인 하는 부분...


    //Tutorial02.fx  파일을 이용해서 픽셀 쉐이더를 로딩하는 부분...
    ...
    //-- Render에서 해 줘도 된다.(지금은 형식이 똑같기 때문에 그냥 이쪽에서 해 준다.
    // Set vertex buffer
    UINT stride = sizeof(SimpleVertex);
    UINT offset = 0;
    g_pImmediateContext->IASetVertexBuffers(0, 1, &g_pVertexBuffer, &stride, &offset);
    //랜더를 위해 GUI에 버텍스버퍼를 올린다.
...
void Render()
{
    ...
    // g_pImmediateContext->IASetVertexBuffers( 0, 1, &g_pVertexBuffer, &stride, &offset );  생략됨 위에서 했으니까....
    // g_pImmediateContext->IASetPrimitiveTopology( D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST );
